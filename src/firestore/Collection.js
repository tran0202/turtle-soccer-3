...
import { Container, Row, Col, Card, CardSubtitle, Form, FormGroup, Label, Button, Table } from 'reactstrap'
...
class Collection extends React.Component {
  ...
  writeData = () => {
    ...
    if (batch) {
      batch.rows.map((row) => {
        const ts = Firebase.firestore.Timestamp.now().toDate().toLocaleString()
        Firebase.firestore()
          .collection(name)
          .doc(row.id)
          ...
          {docs.map((doc, index) => (
            <React.Fragment key={doc.id}>
              ...
              <tr>
                ...
                {displayRow(doc)}
                <td>
                  <Button onClick={() => this.removeData(doc.id)} className="btn btn-link text-white" color="danger" size="sm">
                    Delete
                  </Button>
                </td>
              </tr>
    ...
    return (
      <React.Fragment>
        <Container>
          <Row className="mb-5">
            <Col sm="12">
              <a href="/">Turtle Soccer</a> | <a href="/admin">Firestore Admin Home</a>
            </Col>
          </Row>
          <Row>
            <Col sm="12">
            ...
            <Col sm="3">
              <Form onSubmit={this.handleSubmit}>
                <FormGroup row>
                  ...
                  <Col sm={9}>
                    <input type="text" ref="batch" className="form-control" placeholder="Batch#" onChange={this.showUploadBatch} />
                  </Col>
                </FormGroup>
                <FormGroup check row>
