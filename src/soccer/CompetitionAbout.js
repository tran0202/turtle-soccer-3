...
class CompetitionAbout extends React.Component {
  render() {
    const { tournaments, tournamentType } = this.props
      ...
      <React.Fragment>
        ...
        <Row className="box-xl mb-5">
          <Col>
            ...
            <ResultHead />
            {tournamentType && tournaments && tournaments.map((t, index) => <ResultRow row={t} tt={tournamentType} count={index} key={t.id}></ResultRow>)}
          </Col>
        </Row>
      </React.Fragment>
    )
